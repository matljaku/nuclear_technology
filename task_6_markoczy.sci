...
G = 0.95*Coolant_flow/Number_of_assemblies;
q2p = chAv.q2p;
...
//rho_f = CHAN_fluidProperty(z, 'rho_pT', press, itemp); //[kg/m^3]
...
Nu_DB = DittusBoelter(Re, Pr);
Nu = Markoczy(P, D_co, Re, Pr, Nu_DB);
...
temp_fcen = FuelCenterTemp(D_co,q2p, temp_fout, c, d, D_f);
...
fid = mopen("./data/temp/dz_mar.txt", "w");
for i=1:length(chAv.z)
    print(fid,chAv.z(i));
end
mclose(fid);
fid = mopen("./data/temp/it_mar.txt", "w");
for i=1:length(chAv.z)
    print(fid,itemp(i));
end
mclose(fid);
fid = mopen("./data/temp/temp_cout_mar.txt", "w");
for i=1:length(chAv.z)
    print(fid,temp_cout(i));

end
mclose(fid);
fid = mopen("./data/temp/temp_cin_mar.txt", "w");
for i=1:length(chAv.z)
	 print(fid,temp_cin(i));
end
mclose(fid);

fid = mopen("./data/temp/temp_fout_mar.txt", "w");
for i=1:length(chAv.z)
	 print(fid,temp_fout(i));
end
mclose(fid);
fid = mopen("./data/temp/temp_fcen_mar.txt", "w");
for i=1:length(chAv.z)
...
disp(temp_cin(index_max_fuel));
disp(temp_cout(index_max_fuel));
disp(itemp(index_max_fuel));
halt("Press Enter to continue:");

// Iterations for radial pellet

r_fp = linspace(0.0, D_fo, 10);
disp(r_fp)

fid = mopen("./data/temp/temp_fuel_mar.txt", "w");
for i=1:length(r_fp)
