function [chout]=CHAN_setOperatingConditions(chin,Pref,iin,W,q2p_av,power_shape,grav, rpf)
...
// which must be taken into account in the normalization
  ...
  for i=1:length(Axial_power_distribution)
    disp(Axial_power_distribution(i));
  end
  
  chout.q2p = q2p_av*rpf*Axial_power_distribution;
endfunction
