...
rpf = 1.0;
s= rpf*cos((z-z($)/2)*(5*%pi)/(6*z($)));  // We assume here H/H_tilde = 5/6 and cosine shape
...
fid = mopen("./data/temp/dz_osm.txt", "w");
for i=1:length(chAv.z)
    print(fid,chAv.z(i));
end
mclose(fid);
fid = mopen("./data/temp/it_osm.txt", "w");
for i=1:length(chAv.z)
    print(fid,itemp(i));
end
mclose(fid);
fid = mopen("./data/temp/temp_cout_osm.txt", "w");
for i=1:length(chAv.z)
    print(fid,temp_cout(i));

end
mclose(fid);
fid = mopen("./data/temp/temp_cin_osm.txt", "w");
for i=1:length(chAv.z)
	 print(fid,temp_cin(i));
end
mclose(fid);

fid = mopen("./data/temp/temp_fout_osm.txt", "w");
for i=1:length(chAv.z)
	 print(fid,temp_fout(i));
end
mclose(fid);
fid = mopen("./data/temp/temp_fcen_osm.txt", "w");
for i=1:length(chAv.z)
...
fid = mopen("./data/temp/temp_fuel_osm.txt", "w");
for i=1:length(r_fp)
